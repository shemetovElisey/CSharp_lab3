...
namespace CSharp_Lab_3
{
   class Program
   {
      static void Main()
      {
         ...
         Console.WriteLine("f(pi/6, 7) = {0:F3}", expr.Compute(new Dictionary<string, double> { ["x"] = Math.PI/6, ["y"] = 7 }));
         ...
         Console.WriteLine("f(7pi/3, 6) = {0:F3}", expr.Compute(new Dictionary<string, double> { ["x"] = Math.PI/3, ["y"] = 6 }));
         Console.ReadKey();
      }
   }
}
